...
export async function analyzeContract(text, options = {}) {
  const { country = "US", feedback = null } = options;
  const prompt = buildAnalysisPrompt(text, country, feedback);
...
function buildAnalysisPrompt(contractText, country = "US", userFeedback = null) {
  let countryContext = "";
  if (country && country !== "OTHER") {
    countryContext = `
JURISDICTION CONTEXT:
The user is in ${country}. Consider the legal norms, common contract practices, and relevant laws of this jurisdiction when explaining clauses. Mention any jurisdiction-specific considerations (e.g., enforceability of non-compete clauses, statutory notice periods, consumer protection laws) that may be relevant. Do NOT provide legal advice — only mention well-known general practices for context.
`;
  }

  let userFocusContext = "";
  if (userFeedback) {
    userFocusContext = `
USER'S SPECIFIC FOCUS:
The user has provided the following focus/concern: "${userFeedback}"
Pay special attention to this area. If the contract contains clauses related to the user's concern, prioritize explaining those clauses first and in more detail. Add a "userFocusRelevant" boolean field (true/false) to each clause to indicate if it relates to the user's stated concern.
`;
  }
...
${countryContext}${userFocusContext}
...

{
  ...
  "jurisdiction": "string (detected or specified jurisdiction/country)",
  "keyTerms": {
  ...
  "clauses": [
    {
      ...
      "jurisdictionNote": "string or null (any jurisdiction-specific context, e.g., 'Non-compete clauses have limited enforceability in California')",
      "questionsToConsider": ["string array of questions the user might want to think about"],
      "confidence": "string (one of: high, medium, low)",
      "userFocusRelevant": "boolean (true if this clause relates to the user's stated concern, false otherwise)"
    }
  ],
...
- If the user specified a focus area, prioritize those clauses and give them more detailed explanations.
- Include jurisdiction-specific notes where relevant (but always clarify these are general observations, not legal advice).
